...
int insertRecord(struct Block* block, struct Record rec) {
    for (int i = 0; i < MAX_RECORDS; i++) {
        ...
        //printf("%d\n", block->recordsList[i].TEAM_ID_home);
    }
...
struct Disk* loadData(struct Disk* disk, const char* filename) {
    ...
    fclose(file);
/*    int temp = getRecordFromBlock(getBlock(disk, 0), 0).TEAM_ID_home;
    printf("%d\n", temp);*/
