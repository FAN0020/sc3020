...
struct Disk* createDisk(int diskSize, int blkSize, int lruCacheSize) {
    ...
    for (int i = 0; i < diskSize / blkSize; i++) {
        disk->blocks[i] = createBlock(blkSize); 
...
struct Block* getBlock(struct Disk* disk, int blockNumber) {
    struct Block* block = LRUCache_get(&disk->lruCache, blockNumber);
    if (block != NULL) {
    ...
    if (block == NULL && blockNumber >= 0) {
        ...
        disk->blockAccesses++;
        LRUCache_put(&disk->lruCache, blockNumber, block);
    }
    return block;
}
...
/*int main() {
    ...
    struct Record rec1 = {"tconst1", 7.5, 100};
    struct Record rec2 = {"tconst2", 8.0, 200};
    struct Record rec3 = {"tconst3", 6.5, 50};
